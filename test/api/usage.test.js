...
describe('Usage Model', function() {
  ...
  describe.only('Getting Usage', function () {
    ...
    it('Gets usage for a [limit] user id', function(done) {
      this.kansas.getUsageByOwnerId(this.tokenItem.ownerId).then(function(remaining) {
        expect(remaining).to.equal(7);
      }).then(done, done);
    });
    ...
    it('Gets usage for a [count] user id', function(done) {
      this.kansas.getUsageByOwnerId(this.tokenItemCount.ownerId).then(function(used) {
        expect(used).to.equal(4);
      }).then(done, done);
    });
  });
