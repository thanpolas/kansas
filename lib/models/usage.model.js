...
Usage.prototype._populateChunk = function (chunk, encoding, next) {
  ...
  this.client.hmget(chunk, 'token', 'limit', 'period', function(err, res) {
    ...
    Promise.all([
      self._populateCurrentPeriod(periodName, token, limit),
...
/**
 * Check if current period's key exists, if not create it.
 *
 ...
 */
Usage.prototype._populateCurrentPeriod = Promise.method(function(periodName,
  token, limit) {
  var self = this;
  var bucket = period.get(periodName);
  var usageKey = this.getPrefix() + bucket + ':' + token;
  this.exists(usageKey).then(function(result) {
    if (result) {
      return;
    }
    return self.client.set(usageKey, limit);
  });
});
 ...
 * @return {Promise} A promise.
 */
Usage.prototype._populateNextPeriod = Promise.method(function(periodName,
  token, limit) {
