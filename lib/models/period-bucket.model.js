 ...
 * 
 * @param {kansas.model.period.Unit} period.
 * @return {string} the period bucket.
 */
period.get = function(period) {
  var d = new Date();
 ...
 * 
 * @param {kansas.model.period.Unit} period The period.
...
period.getFuture = function(period, units) {
  ...
  var floored = floordate(d, period);
  switch(period) {
  case period.Unit.DAY:
    floored.setDate(floored.getDate() + units);
    break;
  case period.Unit.WEEK:
    throw new Error('Cannot lookahead when period is "week"');
  case period.Unit.MONTH:
