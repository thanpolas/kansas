...
Token.prototype.validatePolicy = Promise.method(function(userValues) {
  if (!this.policy.has(userValues.policyName)) {
    ...
    log.info('validatePolicy() :: Policy not found:', userValues.policyName,
      'for ownerId:', userValues.ownerId);
    throw err;
  }
});
...
Token.prototype.enforcePolicyLimits = Promise.method(function(userValues) {
  ...
  return this.scard(indexKey).then(function(total) {
    ...
    if (total >= policy.maxTokens) {
      log.info('enforcePolicyLimits() :: Max num of Tokens reached for ownerId:',
...
Token.prototype.getActual = Promise.method(function(token) {
  var keys = this.getKeys(token);
  return this.hgetall(keys.token).then(function(tokenItem) {
    if (__.isObject(tokenItem)) {
      return this.get(keys.usage).then(function(remaining) {
        tokenItem.remaining = remaining;
        return tokenItem;
      });
    } else {
      return tokenItem;
