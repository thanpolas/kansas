...
Token.prototype._createItem = function(token, item) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    function handleCb(err) {
      ...
      self._createUsage(keys, item)
        .bind(self)
        .then(function() {
          resolve(item);
          this.emit(EventBus.Event.CREATE, item);
        })
        .catch(reject);
    }
 ...
 *   - Populate next period's usage key
 *
 * @param {Object} keys The keys to save the usage.
 * @param {Object} item Key value pairs.
 * @return {Promise(number)} A promise with the usage starting point.
 * @private
 */
Token.prototype._createUsage = function(keys, item) {
  var self = this;
  return new Promise(function(resolve, reject) {
    var startingPoint;
    function handleCb(err) {
...
Token.prototype.getActual = Promise.method(function(token) {
  ...
  return this.hgetall(tokenKey)
    ...
    .then(function(tokenItem) {
      ...
      this.normalizeTokenItem(tokenItem);

      var keys = this.getKeys(tokenItem);

      return this.getRedis(keys.usage)
        .then(this._handleUsage.bind(this, tokenItem));
    });
});

/**
 * Handles the usage response from the getActual() method, the main purpose
 * is to handle odd cases where there is no usage record for the specific
 * token that was requested and found to exist, if that is the [odd] case
 * then we go ahead and create the usage records so as to eliminate
 * data corupptness.
 *
 * @param {Object} tokenItem The token item requested.
 * @param {Object|null} result The result from the Redis GET usage operation.
 * @return {Promise(Object)} Returns the tokenItem populated with usage.
 * @private
 */
Token.prototype._handleUsage = function(tokenItem, result) {
  return Promise.resolve()
    .bind(this)
    .then(function() {
      if (!result) {
        var keys = this.getKeys(tokenItem);
        return this._createUsage(keys, tokenItem);
      } else {
        return result;
      }
    })
    .then(function(usageResult) {
      if (tokenItem.count) {
        tokenItem.consumed = usageResult - UsageModel.COUNT_START;
      } else {
        tokenItem.remaining = parseInt(usageResult, 10);
      }
      return tokenItem;
    });
};
