...
var GetUsageModel = require('../models/get-usage.model');
...
Kansas.prototype.connect = Promise.method(function() {
  this.log.finer('connect() :: Init... Connected:', this.connected,
    'Redis host:', this._options.redis.host, 'port:', this._options.redis.port);
  if (this.connected) {
    return;
  }
...
Kansas.prototype._onConnect = function(client) {
  this.log.finer('_onConnect() :: Connected, Instantiating models...');
  this.connected = true;
  this.client = client;
  ...
  this.getUsageModel = new GetUsageModel(client, {prefix: this._options.prefix});
  this.getUsageModel.setTokenModel(this.tokenModel);
  ...
  this.count = this.usageModel.count;

  // Get usage methods
  this.getUsage = this.getUsageModel.getUsage;
  this.getUsageByOwnerId = this.getUsageModel.getUsageByOwnerId;
